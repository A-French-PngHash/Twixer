...
Future<(bool, TweetModel?, String?)> getTweetFromId(int id) async {
  final result = await requestApi(http.get, "/tweet", {"tweet-id": id.toString()});
  if (result.$1) {
...
Future<(bool, List<TweetModel>?, String?)> getTweetOnHomepage(
  ...
  final result = await requestApi(http.get, "/home", {
...
Future<(bool, List<TweetModel>?, String?)> getTweetSearch(
  ...
  final result = await requestApi(http.get, "/search", {
...
Future<(bool, UserModel?, String?)> getProfileDataFor({required String username}) async {
  final result = await requestApi(http.get, "/profile", {
    "username": username,
    "length": "0",
    "offset": "0",
    "order-by": "date",
